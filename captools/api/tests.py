...
class TestClient(unittest.TestCase):
    ...
    def test_client_connection(self):
        ...
        self.assertEqual(client._generate_url('^/api/shreddr/tinker/(?P<id>[\d]+)/(?P<donkey>[^/]+)/$',
                                              [666, 'poundcake']), '/api/shreddr/tinker/666/poundcake/')
        self.assertEqual(client._generate_url('^/(?P<id>[\d]+)/moon/(?P<donkey>[^/]+)$',
                                              [666, 'poundcake']), '/666/moon/poundcake')
        ...
        for i in range(iset['sheet_count']):
            client.create_iset_instance(iset['id'], i, {'image': open(TEST_FORM_IMAGE_PATH)})
        ...
        self.assertTrue(instance_set.get('created', None) is not None,
                        'Instance set has no created attribute %s' % instance_set)
